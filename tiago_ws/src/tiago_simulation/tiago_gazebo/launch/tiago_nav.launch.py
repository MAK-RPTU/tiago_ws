...
# from launch_pal.robot_arguments import CommonTiagoArgs
from launch_pal.robot_arguments import CommonTiagoArgs
...
class LaunchArguments(LaunchArgumentsBase):
    ...
    navigation: DeclareLaunchArgument = CommonTiagoArgs.navigation
    advanced_navigation: DeclareLaunchArgument = CommonTiagoArgs.advanced_navigation
    slam: DeclareLaunchArgument = CommonTiagoArgs.slam
    moveit: DeclareLaunchArgument = CommonTiagoArgs.moveit
    world_name: DeclareLaunchArgument = CommonTiagoArgs.world_name
    namespace: DeclareLaunchArgument = CommonTiagoArgs.namespace
    tuck_arm: DeclareLaunchArgument = CommonTiagoArgs.tuck_arm
    is_public_sim: DeclareLaunchArgument = CommonTiagoArgs.is_public_sim

    use_gazebo: DeclareLaunchArgument = DeclareLaunchArgument(
        'use_gazebo',
        default_value='false',
        description="Enable/Disable Gazebo simulation"
    )

    headless: DeclareLaunchArgument = DeclareLaunchArgument(
        'headless',
        default_value='false',  # headless mode
...
def declare_actions(
    ...
    # Check if we give the input argument as true for use_gazebo it goes inside else not run gazebo
    # if launch_args.use_gazebo:
    ...
    gazebo_headless = include_scoped_launch_py_description(
        pkg_name='pal_gazebo_worlds',
        paths=['launch', 'pal_gazebo_headless.launch.py'],
        env_vars=[gazebo_model_path_env_var],
        launch_arguments={
            "world_name":  launch_args.world_name,
            "model_paths": packages,
        ...
        # condition=IfCondition(PythonExpression(["'", LaunchConfiguration("use_gazebo"), "' == 'false'"]))
        condition=IfCondition(LaunchConfiguration('headless'))
    )

    launch_description.add_action(gazebo_headless)
